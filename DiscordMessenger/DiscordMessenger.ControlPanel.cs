...
namespace NinjaTrader.NinjaScript.Indicators
{
    public partial class DiscordMessenger : Indicator
    {
        ...
        private Task HandleScreenshot(ProcessType processType)
        {
            return TakeScreenshot(processType);
        }

        private Task TakeScreenshot(ProcessType processType)
        {
            DateTime now = DateTime.Now;
            string screenshotName = now.Year.ToString() + 
                ...
                try
                {
                    if (_chartWindow != null)
                    {
                        ...
                        if (screenCapture != null)
                        {
                            ...
                            _controlPanelEvents.ScreenshotProcessed(processType, screenshotName);
                        }
                    }
                    tcs.SetResult(0);
                }
