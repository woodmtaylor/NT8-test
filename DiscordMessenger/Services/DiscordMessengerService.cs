    ...
    public class DiscordMessengerService
    {
        ...
        private void HandleOnOrderEntryProcessed(List<Position> positions, List<OrderEntry> orderEntries)
        {
            ...
            Task.Run(delegate()
            {
                ...
                Task.Delay(500).Wait();
                _controlPanelEvents.TakeScreenshot(ProcessType.Auto);
            });
        }
        ...
        {
            SendMessageAsync(delegate(bool success, string message)
            {
                if (success)
                {
        ...
        private Task SendMessageAsync(Action<bool, string> callback)
        {
            ...
            return SendHttpRequestAsync(jsonPayload, callback);
        }
        ...
        private Task HandleOnScreenshotProcessed(ProcessType processType, string screenshotName)
        {
            ...
            else
            {
                ...
                return SendScreenshotAsync(delegate(bool success, string message)
                {
                    if (success)
                    {
                        _eventLoggingEvents.SendRecentEvent(new EventLog
