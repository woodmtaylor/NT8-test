...
namespace NinjaTrader.Custom.AddOns.DiscordMessenger.UserInterfaces.Components
{
    public class RecentEventsGrid : Grid, IComponentSetup
    {
        ...
        {
            _eventLogsListlabel.Dispatcher.Invoke(new System.Action(delegate()
            {
                _eventLogsListlabel.Content = string.Empty;

                for (int i = eventLogs.Count - 1; i >= 0; i--)
                {
                    EventLog log = eventLogs[i];
                    string logEntry = log.Status + " | " + log.Time.ToString("HH:mm:ss") + " | " + log.Message;
                    _eventLogsListlabel.Content = _eventLogsListlabel.Content + logEntry + "\n";
                }
            }));
        }
    }
